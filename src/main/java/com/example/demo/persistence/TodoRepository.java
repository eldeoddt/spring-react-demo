...
public interface TodoRepository extends JpaRepository<TodoEntity, String> {
    ...
    // 자동으로 save(), delete(), findbyid() 메서드 등 자동으로 생성됨.
    // findbytitle, findbyuserid은 자동 제공이 아니다. todorepo가 자동으로 생성될 때 findbyid는 자동 생성해준다.

    @Query(value = "select * from todo t where t.userId = ?1")// 이 메서드가 실행되면 이 쿼리가 실행되도록 한다.
    // 물은표 1은 첫 번째 인자로 들어온게 userid와 일치하는 행을 모두 검색해라는 뜻이다.
    public TodoEntity findByUserId(String userId); // 사용하려면 선언해줘야 한다.
